...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "3d object", NULL, NULL);
	if (window == NULL)
	{
	...
	Shader shader("../shaders/cube_v.glsl", "../shaders/cube_f.glsl");
	shader.use();
	...
	projection = glm::perspective(glm::radians(45.00f), float(SCREEN_WIDTH / SCREEN_HEIGHT), 0.1f, 100.f);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader.use();
		model = glm::rotate(model, float(glfwGetTime())/1000, glm::vec3(0.5f, 1.0f, 0.0f));
